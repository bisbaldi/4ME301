...
with open('../SWordTimestamp.txt', 'r') as timestamp:
...
with open('../words.csv', 'a') as lessons:
    ...
    for p in content['vocab_overview']:
        ...
        if time > last_time_stamp:
            word_writer.writerow([time, language, word_string.encode('utf8'), word_id, lexeme_id, strength, strength_bar, word_type,
                                  gender, related_lexemes, normalized_string.encode('utf8'), infinitive, skill, skill_url])
            if time > max_time:
                max_time = time

# WRITE TIMESTAMP FOR LAST FETCHED DATA
with open('../SWordTimestamp.txt', 'w') as timestamp:
